...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const saved = localStorage.getItem('padel_player');
  if (saved) {
    try {
...
function loginSuccess(player, source = 'session_start') {
  ...
  ST.player = player;
  localStorage.setItem('padel_player', JSON.stringify(player));
  showView('app');
...
async function switchUser(playerId) {
  ...
  ST.players = [];
  localStorage.setItem('padel_player', JSON.stringify(data));
  loginSuccess(data);
}
  ...
  showFormModal('Terms of Use', `
    <div style="font-size:14px;line-height:1.6;color:#333">
      ...
      <ol style="padding-left:18px;margin-top:10px;display:flex;flex-direction:column;gap:10px">
        <li><strong>Informal tool.</strong> This app is provided as a convenience for members of the BC Padel Section. It is not an official British Club service.</li>
